...
func looseScheme(t *testing.T) Scheme {
	t.Helper()
	scheme, ok := ByName(policy.VersioningLoose, "")
	assert.True(t, ok)
	return scheme
}
...
func TestByName(t *testing.T) {
	...
	for _, name := range []policy.Versioning{"", policy.VersioningSemver} {
		...
		assert.Equal(t, policy.VersioningSemver, scheme.Name())
	}

	scheme, ok := ByName(policy.VersioningLoose, "")
	assert.True(t, ok)
	assert.Equal(t, policy.VersioningLoose, scheme.Name())

	_, ok = ByName("lose", "")
	assert.False(t, ok)

	scheme, ok = ByName(policy.VersioningRegex, calendarPattern)
	assert.True(t, ok)
	...
	_, ok = ByName(policy.VersioningRegex, "")
	assert.False(t, ok, "regex without a pattern reads nothing")
	_, ok = ByName(policy.VersioningRegex, `^(?P<major>\d+$`)
	assert.False(t, ok, "a pattern that does not compile")
	_, ok = ByName(policy.VersioningRegex, `^\d+$`)
...
func regexSchemeFrom(t *testing.T, pattern string) Scheme {
	t.Helper()
	scheme, ok := ByName(policy.VersioningRegex, pattern)
...
func TestRegexVersioningOrder(t *testing.T) {
	...
	patch, minor, major := LatestPerLevel(scheme, "2024-02-01", ordered)
	...
	assert.Equal(t, "", Latest(scheme, "sha-e1c83ba", ordered, true, true, true))
}
